...
def seed(
    register_user_endpoint: str,
    tweet_endpoint: str,
    user_amount: int,
    tweet_per_user_amount: int,
) -> Tuple[bool, List[Response], List[Response]]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "register_endpoint", type=str, help="The endpoint for user registration"
    )
    parser.add_argument(
        "tweet_endpoint", type=str, help="The endpoint for posting tweets"
    )
    parser.add_argument("user_amount", type=int, help="The number of users to register")
    parser.add_argument(
        "tweet_per_user_amount", type=int, help="The number of tweets per user"
    )

    args = parser.parse_args()

    (isSuccess, register_user_results, tweet_results) = seed(
        args.register_endpoint,
        args.tweet_endpoint,
        args.user_amount,
        args.tweet_per_user_amount,
    )

    if isSuccess:
        print(
            f"Success, registered {args.user_amount} users and each user tweeted {args.tweet_per_user_amount}"
        )
    else:
        register_user_errors = [
            result for result in register_user_results if not result.ok
        ]

        if len(register_user_errors) > 0:
            print(
                f"Register user returned {len(register_user_errors)} errors, printing the first 5"
            )
        ...
        if len(tweet_errors) > 0:
            print(
                f"Tweet for a user returned {len(tweet_errors)} errors, printing the first 5"
            )
